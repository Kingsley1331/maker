...
import { createBody, createPolygon, DEFAULT_SIZE, type Point, type ShapeType } from "./shapes";
 ...
 */
export function setupInput({ engine, render, isPaused, getSelectedShape, onSelectionUpdate }: InputOptions): Input {
  ...
  function isPolygonTool(): boolean {
    return getSelectedShape() === "polygon";
  }
  ...
  let draft: Point[] = [];
  /** Cursor position while the polygon tool is selected, for the rubber-band preview. */
  let hover: Point | null = null;

  function applyCursor(): void {
    if (moveOffset) render.canvas.style.cursor = "grabbing";
    else if (isPolygonTool()) render.canvas.style.cursor = "crosshair";
  ...
  function rebuildGhost(size: number): void {
    ...
    const type = getSelectedShape();
    if (type === "polygon") return;
    ghost = createBody(type, spawnStart.x, spawnStart.y, size);
  ...
  function reset(): void {
    ...
    moveOffset = null;
    if (draft.length === 0) setGrabEnabled(true);
    applyCursor();
  ...
  function onMove(event: MouseEvent): void {
    ...
    if (!spawnStart || isPolygonTool()) return;
  ...
  function onUp(event: MouseEvent): void {
    ...
    if (isPolygonTool()) {
      ...
      if (dragged && ghost) {
        ...
        const type = getSelectedShape();
        if (type !== "polygon") {
          Composite.add(engine.world, createBody(type, spawnStart.x, spawnStart.y, DEFAULT_SIZE));
        }
      }
    }
